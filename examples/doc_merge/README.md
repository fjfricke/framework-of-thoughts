# Doc Merge (NDA Task)

## Setup

The dataset is provided in the `dataset` folder.
They are copied from the [graph-of-thoughts](https://github.com/spcl/graph-of-thoughts) repository (© 2023 ETH Zurich). For more details, refer to the [license file](dataset/license.txt).

## Executions

To execute the dataset evaluation, run `got_dataset_evaluation.py` with the parameters set in the if __name__ == "__main__" block.

To execute the dspy study, run `dspy_study.py`.

## Output

The results of the dataset evaluation are saved in the `output` folder.